...
class PolicyDriverManager(stevedore.named.NamedExtensionManager):
    ...
    def create_contract_precommit(self, context):
        """Notify all policy drivers during contract creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_contract_precommit call fails.
    ...
    def create_contract_postcommit(self, context):
        """Notify all policy drivers after contract creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_contract_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where the contract will be deleted, triggering
        any required cleanup. There is no guarantee that all policy
        drivers are called in this case.
        """
    ...
    def update_contract_precommit(self, context):
        """Notify all policy drivers during contract update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_contract_precommit call fails.
    ...
    def update_contract_postcommit(self, context):
        """Notify all policy drivers after contract update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_contract_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where an error is returned to the user. The
        user is expected to take the appropriate action, whether by
        retrying the call or deleting the contract. There is no
        guarantee that all policy drivers are called in this case.
        """
        self._call_on_drivers("update_contract_postcommit", context)

    def delete_contract_precommit(self, context):
        """Notify all policy drivers during contract deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_contract_precommit call fails.
    ...
    def delete_contract_postcommit(self, context):
        """Notify all policy drivers after contract deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_contract_postcommit call fails.
        ...
        then reraised at the end to notify the caller of a failure. In
        general we expect the caller to ignore the error, as the
        contract resource has already been deleted from the database
        and it doesn't make sense to undo the action by recreating the
        contract.
        """
    ...
    def create_policy_rule_precommit(self, context):
        """Notify all policy drivers during policy_rule creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_policy_rule_precommit call fails.
    ...
    def create_policy_rule_postcommit(self, context):
        """Notify all policy drivers after policy_rule creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_policy_rule_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where the policy_rule will be deleted, triggering
        any required cleanup. There is no guarantee that all policy
        drivers are called in this case.
        """
    ...
    def update_policy_rule_precommit(self, context):
        """Notify all policy drivers during policy_rule update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_policy_rule_precommit call fails.
    ...
    def update_policy_rule_postcommit(self, context):
        """Notify all policy drivers after policy_rule update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_policy_rule_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where an error is returned to the user. The
        user is expected to take the appropriate action, whether by
        retrying the call or deleting the policy_rule. There is no
        guarantee that all policy drivers are called in this case.
        """
        self._call_on_drivers("update_policy_rule_postcommit", context)

    def delete_policy_rule_precommit(self, context):
        """Notify all policy drivers during policy_rule deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_policy_rule_precommit call fails.
    ...
    def delete_policy_rule_postcommit(self, context):
        """Notify all policy drivers after policy_rule deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_policy_rule_postcommit call fails.
        ...
        then reraised at the end to notify the caller of a failure. In
        general we expect the caller to ignore the error, as the
        policy_rule resource has already been deleted from the database
        and it doesn't make sense to undo the action by recreating the
        policy_rule.
        """
    ...
    def create_policy_classifier_precommit(self, context):
        """Notify all policy drivers during policy_classifier creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_policy_classifier_precommit call fails.
    ...
    def create_policy_classifier_postcommit(self, context):
        """Notify all policy drivers after policy_classifier creation.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver create_policy_classifier_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where the policy_classifier will be deleted, triggering
        any required cleanup. There is no guarantee that all policy
        drivers are called in this case.
        """
    ...
    def update_policy_classifier_precommit(self, context):
        """Notify all policy drivers during policy_classifier update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_policy_classifier_precommit call fails.
    ...
    def update_policy_classifier_postcommit(self, context):
        """Notify all policy drivers after policy_classifier update.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver update_policy_classifier_postcommit call fails.

        Called after the database transaction. If a policy driver
        raises an exception, then a GroupPolicyDriverError is propagated
        to the caller, where an error is returned to the user. The
        user is expected to take the appropriate action, whether by
        retrying the call or deleting the policy_classifier. There is no
        guarantee that all policy drivers are called in this case.
        """
        self._call_on_drivers("update_policy_classifier_postcommit", context)

    def delete_policy_classifier_precommit(self, context):
        """Notify all policy drivers during policy_classifier deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_policy_classifier_precommit call fails.
    ...
    def delete_policy_classifier_postcommit(self, context):
        """Notify all policy drivers after policy_classifier deletion.

        :raises: neutron.plugins.grouppolicy.common.GroupPolicyDriverError
        if any policy driver delete_policy_classifier_postcommit call fails.
        ...
        then reraised at the end to notify the caller of a failure. In
        general we expect the caller to ignore the error, as the
        policy_classifier resource has already been deleted from the database
        and it doesn't make sense to undo the action by recreating the
        policy_classifier.
        """
