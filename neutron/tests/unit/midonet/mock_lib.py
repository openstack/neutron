...
def get_rule_mock(chain_id, action, id=None, tenant_id='test-tenant',
    ...
    attrs = {'dl_src': 'get_dl_src',
             'dl_type': 'get_dl_type',
             'inv_dl_src': 'get_inv_dl_src',
             ...
             'match_return_flow': 'is_match_return_flow',
             'nw_dst_address': 'get_nw_dst_address',
             'nw_dst_length': 'get_nw_dst_length',
    ...
    for k, v in kwargs.iteritems():
        if k in attrs:
            if attrs[k] is not None:
                getattr(r, attrs[k]).return_value = v
        else:
            raise Exception("Unrecognized kwarg: (%s, %s)" % (k, v))
...
class MidoClientMockConfig():
    ...
    def _get_chain_by_name(self, tenant_id, name):
        for chain in self.chains.values():
            if chain.get_name() == name and chain.get_tenant_id() == tenant_id:
                return chain
        return None
    ...
    def _add_chain_rule(self, chain, action, query=None, **kwargs):
        tenant_id = self._get_query_tenant_id(query)
        r = get_rule_mock(
            chain.get_id(), action, tenant_id=tenant_id, **kwargs)
