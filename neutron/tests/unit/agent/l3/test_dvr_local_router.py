...
class TestDvrRouterOperations(base.BaseTestCase):
    ...
    def test__load_used_fip_information(self):
        ...
        with mock.patch.object(dvr_fip_ns.FipNamespace,
                ...
                mock.patch.object(router, 'get_floating_ips',
                                  return_value=fips):
            IpRuleMock = mock.Mock()
            self.mock_rule.rule = IpRuleMock
            router._load_used_fip_information()

        IpRuleMock.add.assert_called_once_with(
            ip=fips[-1]['fixed_ip_address'], table=dvr_fip_ns.FIP_RT_TBL,
            priority=fip_rule_prio_list[-1][2])
        self.assertEqual(3, len(router.floating_ips_dict))
        ret = [[fip, fixed_ip, p] for fip, (fixed_ip, p) in
