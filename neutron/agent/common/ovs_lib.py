...
class OVSBridge(BaseOVS):
    ...
    def _update_queue(self, port_id, queue_num, queue_type, max_kbps=None,
        ...
        other_config = {}
        if max_kbps:
            other_config[six.u('max-rate')] = str(
                int(max_kbps) * p_const.SI_BASE)
            other_config[six.u('burst')] = str(
                int(max_burst_kbps) * p_const.SI_BASE)
        if min_kbps:
            other_config[six.u('min-rate')] = str(min_kbps * p_const.SI_BASE)
